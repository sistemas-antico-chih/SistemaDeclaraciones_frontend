  ...
  validarFechas(fechaIngreso: string, fechaEgreso: string) {
    return (formGroup: FormGroup) => {
      ...
      if (fegreso.errors && !fegreso.errors.validarFechas) {
        console.log('fechaIngresoParse 1: '+Date.parse(fingreso.value));
        console.log('fechaEgresoParse 1: '+Date.parse(fegreso.value));
        return;
      }

      if (Date.parse(fingreso.value) >= Date.parse(fegreso.value)) {
        console.log('fechaIngresoParse 2: '+Date.parse(fingreso.value));
        console.log('fechaEgresoParse 2: '+Date.parse(fegreso.value));
        fegreso.setErrors({ validarFechas: true });
      }
      else {
        console.log('fechaIngresoParse 3: '+Date.parse(fingreso.value));
        console.log('fechaEgresoParse 3: '+Date.parse(fegreso.value));
